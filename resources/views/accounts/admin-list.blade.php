        ...
        <div class="flex">
            ...
            <div id="dropdownAction" class="z-10 hidden bg-white divide-y divide-zinc-100 rounded-lg shadow w-44 dark:bg-zinc-700 dark:divide-zinc-600">
                <ul class="py-1 text-sm text-zinc-700 dark:text-zinc-200" aria-labelledby="dropdownActionButton">
                    <li>
                        <form action="{{route('accounts.admin.list')}}" method="GET">
                    ...
                    <li>
                        ...
                        <a href="{{route('accounts.admin.list')}}" class="w-full text-start flex items-center px-4 py-2 hover:bg-zinc-100 hover:text-zinc-800 dark:hover:bg-zinc-600 dark:hover:text-white">Descending
            ...
            <div class="ms-4 md:ms-8 w-fit border-[1.5px] dark:border-zinc-700 px-3 py-2 m-0 rounded text-zinc-900 dark:text-white">
                Total : <span class=" font-semibold text-primary">{{$admins->total()}}</span>
            </div>
        </div>


        <form class="mt-3 md:mt-0 relative" action="{{route('accounts.admin.list')}}" method="GET">
        ...
        <tbody>
            ...
            <tr class="text-zinc-900 dark:text-white bg-white border-b dark:bg-zinc-800 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-600">
                ...
                <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                    ...
                    <img class="w-10 h-10 rounded-full object-cover" src="{{asset('storage/'.$admin->profile_photo_path)}}" alt="Jese image">
                    @else
                ...
                <td class="px-6 py-4">
                   ...
                   <div class="flex items-center">
                    <form action="{{route('accounts.changeRole')}}" method="POST">
                        ...
                        <button type="submit" data-id="" class="btn-edit mt-1">
                    ...
                    <button data-modal-target="default-modal" data-modal-toggle="default-modal" type="button" class="ms-1 btn-delete">
                        <div data-popover id="popover-delete" role="tooltip" class="absolute z-10 invisible inline-block text-sm text-zinc-500 transition-opacity duration-300 bg-white border border-zinc-200 rounded-lg shadow-sm opacity-0 dark:text-zinc-400 dark:border-zinc-600 dark:bg-zinc-800">
                            ...
                            <div class="px-3 py-2">
                                <p>Delete<span class="font-semibold"> admin</span> account from table.</p>
                            </div>
                            <div data-popper-arrow></div>
                        </div>
                        <svg  class="text-slate-600 dark:text-slate-50" data-popover-target="popover-delete" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M12 2c5.53 0 10 4.47 10 10s-4.47 10-10 10S2 17.53 2 12S6.47 2 12 2m5 5h-2.5l-1-1h-3l-1 1H7v2h10V7M9 18h6a1 1 0 0 0 1-1v-7H8v7a1 1 0 0 0 1 1Z"/></svg>
                    </button>
                    <!-- Main modal -->
                        ...
                        <div class="relative w-full max-w-2xl max-h-full flex justify-center items-center">
                            ...
                            <div class="animate__animated animate__bounceIn relative w-full md:w-2/3 bg-white rounded-lg shadow dark:bg-zinc-700">
                                ...
                                <div class="p-6 space-y-6">
                                    ...
                                    You won't be able to revert this. This admin account will be deleted forever.
                                </p>
                                </div>
                                <!-- Modal footer -->
...
<div class="mt-4 px-2 pagination">{{$admins->appends(request()->query())->links()}}</div>
